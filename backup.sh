...
RELEASE_ID=$(curl -s -H "Authorization: Bearer $GH_TOKEN" "$API_BASE/releases/tags/latest" | jq -r '.id // empty')
...
OLD_ASSETS_JSON=$(curl -s -H "Authorization: Bearer $GH_TOKEN" "$API_BASE/releases/$RELEASE_ID/assets" \
...
if [ "$OLD_ASSETS_JSON" != "[]" ] && [ "$OLD_ASSETS_JSON" != "null" ]; then
    echo "$OLD_ASSETS_JSON" | jq -c '.[]' | while read -r asset; do
        ...
        curl -s -X DELETE -H "Authorization: Bearer $GH_TOKEN" "$API_BASE/releases/assets/$ASSET_ID" >/dev/null
    done
fi

b64_encode() { base64 -w 0 2>/dev/null || base64; }

README_SHA=$(curl -s -H "Authorization: Bearer $GH_TOKEN" \
